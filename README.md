...
| 版本 | 特性 | 核心组件 |
|------|------|----------|
| v9.8.0 | iOS SDK对齐Android | ErrorHandler、CircuitBreaker、EventBus、CrossDeviceRouter、Memory L1/L2/L3/Archive |
| v9.7.0 | Dashboard监控界面 | Scenes.vue场景可视化、ApiDebugger.vue接口调试、WebSocket实时更新 |
| v9.6.0 | 性能优化 + 压测验证 | ConnectionPool、CacheManager、BatchProcessor、AsyncWorkerPool、性能提升5x |
| v9.5.0 | SDK-Center联调验证 | WebSocket集成测试、自动化脚本、场景验证 |
| v9.4.0 | SDK增强 + 场景扩展 | ErrorHandler、CircuitBreaker、DrivingDetector、ExerciseDetector、SleepDetector、WorkDetector、HomeDetector |
