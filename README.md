...
| 特性 | 描述 |
|------|------|
| **PostgreSQL 持久化** | 支持 PostgreSQL 数据库存储，企业级数据持久化 |
| **Redis 缓存层** | 高性能缓存，支持在线状态、会话、Pub/Sub |
| **用户画像系统** | MBTI性格、价值观、兴趣爱好、记忆库、偏好系统 |
| **扩展 App 适配器** | 抖音、小红书、滴滴出行自动化支持 |
| **Web Dashboard** | 用户画像可视化，性格特质图表，价值观雷达图 |
...
#### 🚀 v1.4 New Features - Data Persistence & User Profile

| Feature | Description |
|---------|-------------|
| **PostgreSQL Persistence** | Enterprise-grade data persistence with PostgreSQL |
| **Redis Cache Layer** | High-performance caching for online status, sessions, Pub/Sub |
| **User Profile System** | MBTI personality, values, interests, memory, preferences |
| **Extended App Adapters** | Douyin, Xiaohongshu, Didi automation support |
| **Web Dashboard** | User profile visualization, personality charts, value radar |

#### 📱 v1.0 Features - MCP Protocol Support
