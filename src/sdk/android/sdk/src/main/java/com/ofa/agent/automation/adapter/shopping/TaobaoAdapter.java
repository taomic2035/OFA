...
/**
 * Taobao (淘宝) App Adapter.
 * Package: com.taobao.taobao
 */
public class TaobaoAdapter extends BaseAppAdapter {

    private static final String TAG = "TaobaoAdapter";

    // Package names
    private static final String PACKAGE_MAIN = "com.taobao.taobao";
    ...
    public float canHandle(@NonNull AutomationEngine engine) {
        ...
        if (pkg.equals(PACKAGE_MAIN) || pkg.startsWith("com.taobao")) {
            // Check for Taobao-specific UI elements
            if (hasText(engine, "淘宝") || hasText(engine, "天猫") ||
                hasText(engine, "购物") || hasText(engine, "店铺")) {
                return 0.9f;
            }
            return 0.7f;
        }
    ...
    public String detectPage(@NonNull AutomationEngine engine) {
        // Order page
        if (hasText(engine, "订单详情") || hasText(engine, "物流信息") ||
            hasText(engine, "已发货") || hasText(engine, "已签收")) {
            return PAGE_ORDER;
        }

        // Checkout page
        if (hasText(engine, "提交订单") || hasText(engine, "确认订单") ||
            hasText(engine, "立即付款")) {
            return PAGE_CHECKOUT;
        }

        // Cart page
        if (hasText(engine, "购物车") && (hasText(engine, "结算") ||
            hasText(engine, "全选"))) {
            return PAGE_CART;
        }

        // Product detail page
        if (hasText(engine, "立即购买") || hasText(engine, "加入购物车") ||
            hasText(engine, "宝贝详情") || hasText(engine, "评价")) {
            return PAGE_PRODUCT;
        }

        // Shop page
        if (hasText(engine, "进店") || hasText(engine, "店铺") ||
            hasText(engine, "宝贝") || hasText(engine, "全部宝贝")) {
            return PAGE_SHOP;
        }

        // Search page
        if (hasText(engine, "搜索") && (hasText(engine, "宝贝") ||
        ...
        if (hasText(engine, "首页") || hasText(engine, "推荐") ||
            hasText(engine, "关注") || hasText(engine, "直播")) {
            return PAGE_HOME;
        }
    ...
    public AutomationResult search(@NonNull AutomationEngine engine, @NonNull String query) {
        ...
        if (!detectPage(engine).equals(PAGE_SEARCH)) {
            BySelector searchSelector = BySelector.textContains("搜索")
                .orId("search_button").orId("home_search");
        ...
        BySelector searchButtonSelector = BySelector.text("搜索")
            .orId("search_go").orId("btn_search");
    ...
    public AutomationResult selectShop(@NonNull AutomationEngine engine, @NonNull String shopName) {
        ...
        // Try to find shop in results
        BySelector shopSelector = BySelector.textContains(shopName)
            .orDescContains(shopName);

        AutomationNode shopNode = engine.findElement(shopSelector);
        if (shopNode != null) {
            AutomationResult result = engine.click(shopNode.getCenterX(), shopNode.getCenterY());
            if (result.isSuccess()) {
    ...
    public AutomationResult addToCart(@NonNull AutomationEngine engine, int quantity) {
        ...
        if (quantity > 1) {
            BySelector quantitySelector = BySelector.textContains("数量")
                .orId("quantity_text").orId("tv_quantity");

            AutomationNode quantityNode = engine.findElement(quantitySelector);
            if (quantityNode != null) {
                // Click to open quantity selector
                engine.click(quantityNode.getCenterX(), quantityNode.getCenterY());
        ...
        String[] addTexts = {"加入购物车", "加入购物车", "添加到购物车"};
        ...
        // Try by ID
        String[] addIds = {"add_to_cart", "btn_add_cart", "add_cart_btn"};
    ...
    public AutomationResult goToCart(@NonNull AutomationEngine engine) {
        ...
        // Try cart icon
        String[] cartIds = {"cart_icon", "shopping_cart", "tab_cart"};
    ...
    public AutomationResult goToCheckout(@NonNull AutomationEngine engine) {
        ...
        String[] checkoutTexts = {"结算", "去结算", "立即购买"};
    ...
    public AutomationResult selectAddress(@NonNull AutomationEngine engine,
        ...
        BySelector addressSelector = BySelector.textContains("地址")
            .orTextContains("收货").orId("address_selector");
    ...
    public AutomationResult submitOrder(@NonNull AutomationEngine engine) {
        ...
        String[] submitTexts = {"提交订单", "确认订单", "提交"};
    ...
    public OrderStatus getOrderStatus(@NonNull AutomationEngine engine, @Nullable String orderId) {
        String[] deliveredTexts = {"已签收", "已收货", "已完成", "交易成功"};
        String[] deliveringTexts = {"运输中", "派送中", "正在派送"};
        String[] shippedTexts = {"已发货", "卖家已发货"};
