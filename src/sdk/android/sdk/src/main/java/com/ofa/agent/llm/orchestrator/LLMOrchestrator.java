...
public class LLMOrchestrator implements LLMProvider {
    ...
    public CompletableFuture<float[]> embed(@NonNull String text) {
        return executeEmbedWithFailover(provider -> provider.embed(text));
    }
     ...
     */
    @FunctionalInterface
    private interface EmbedOperation {
        CompletableFuture<float[]> execute(LLMProvider provider);
    }

    /**
     * 带故障转移的嵌入执行
     */
    private CompletableFuture<float[]> executeEmbedWithFailover(EmbedOperation operation) {
        LLMProvider provider = getActiveProvider();
        if (provider == null) {
            return CompletableFuture.completedFuture(new float[0]);
        }

        CompletableFuture<float[]> future = operation.execute(provider);

        if (autoFailover) {
            return future.thenApply(embedding -> {
                recordSuccess(provider);
                return embedding;
            }).exceptionally(throwable -> {
                recordFailure(provider);
